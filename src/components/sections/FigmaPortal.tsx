...
export function FigmaPortal() {
  ...
  const [submittedFigmaUrl, setSubmittedFigmaUrl] = React.useState("");
  ...
  // Core verification code sending logic (extracted for reuse)
  ...
  // Figma URL validation helper
  const isValidFigmaUrl = (url: string) => {
    const clean = url.trim().toLowerCase();
    if (!clean) return false;
    return (
      clean.includes("figma.com/") ||
      clean.startsWith("https://www.figma.com") ||
      clean.startsWith("https://figma.com") ||
      clean.startsWith("http://figma.com")
    );
  };

  // Handle Step 3: Final Upload & Submission (Figma Design Link)
  const handleSubmitFigma = async (e: React.FormEvent) => {
    e.preventDefault();
    setErrorMessage("");

    const cleanFigmaUrl = figmaUrl.trim();
    if (!cleanFigmaUrl) {
      setErrorMessage("Please enter your Figma prototype/design link.");
      return;
    }

    if (!isValidFigmaUrl(cleanFigmaUrl)) {
      setErrorMessage("Invalid Figma link format. Please provide a valid Figma URL (e.g. https://www.figma.com/file/..., https://www.figma.com/proto/..., or https://www.figma.com/design/...).");
      return;
    }
    ...
    try {
      ...
      if (resData.success) {
        setSubmissionId(resData.submissionId || `NOVA-UIUX-${Math.floor(100000 + Math.random() * 900000)}`);
        setSubmissionTimestamp(resData.timestamp || new Date().toLocaleString());
        setSubmittedFigmaUrl(cleanFigmaUrl);
        setCurrentStep(4);
      } else {
    ...
    <div className="relative min-h-screen bg-[#000d21] text-[#f7fafc] font-sans overflow-x-hidden selection:bg-[#ffb81c]/30">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-7 bg-white border border-gray-200 rounded-3xl p-6 sm:p-10 backdrop-blur-xl shadow-xl flex flex-col justify-between min-h-[620px]">
            <div>
              ...
              <div className="flex items-center justify-center gap-3 sm:gap-6 mb-10 font-space select-none">
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-black transition-all ${
                      currentStep >= 1
                        ? `bg-[${novaBlue}] text-white shadow-md`
                ...
                <div className={`h-[1px] w-8 sm:w-16 transition-colors ${currentStep > 1 ? `bg-[${novaBlue}]` : "bg-gray-200"}`} />
                ...
                <div className={`h-[1px] w-8 sm:w-16 transition-colors ${currentStep > 2 ? `bg-[${novaBlue}]` : "bg-gray-200"}`} />
                ...
                <form onSubmit={handleSendCode} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-gray-50 border border-gray-300 focus:border-[#003599] rounded-2xl px-5 py-4 text-sm text-black placeholder-gray-400 outline-none transition-all focus:ring-2 focus:ring-[#003599]/10"
                    />
                    <p className="text-[10px] text-black mt-1.5 font-sans">
                      Only team leaders are authorized to submit UI/UX prototypes on behalf of their team.
                    </p>
                  </div>
                ...
                <form onSubmit={handleVerifyCode} className="space-y-6 text-center">
                  ...
                  <div className="flex items-center justify-between pt-4 font-space text-xs">
                    <button
                      ...
                      className="border border-gray-300 hover:border-[#003599] bg-gray-100 text-black px-4 py-2 rounded-xl uppercase tracking-wider font-bold transition-all"
                    >
                      EDIT DETAILS
                    </button>
              ...
              {/* STEP 3: FIGMA PROTOTYPE SUBMISSION */}
              {currentStep === 3 && (
                <form onSubmit={handleSubmitFigma} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      placeholder="https://www.figma.com/file/..., https://www.figma.com/proto/..., or https://www.figma.com/design/..."
                    ...
                    <p className="text-[10px] text-black font-sans">
                      Provide the link to your finalized UI/UX prototype or design file on Figma. Ensure sharing permissions allow viewing (Anyone with the link can view).
                    </p>
                  </div>
                ...
                <div className="text-center space-y-6 py-6 font-space animate-fade-in">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-black uppercase tracking-widest font-bold">
                      Your UI/UX prototype has been officially registered
                    </p>
                  </div>
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6 text-left space-y-3 font-sans text-xs">
                    ...
                    {submittedFigmaUrl && (
                      <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                        <span className="text-black">FIGMA PROTOTYPE LINK:</span>
                        <a
                          href={submittedFigmaUrl}
                          target="_blank"
                          rel="noreferrer"
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="w-full sm:w-auto border border-[#003599] bg-[#003599] text-white text-xs font-bold uppercase tracking-widest px-6 py-3.5 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 hover:bg-[#002877]"
                    >
          ...
          {/* RIGHT PANEL: Submission Guidelines & Template (5 Cols on desktop) */}
          <div className="lg:col-span-5 bg-[#001433]/80 border border-[#003599]/50 rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-[0_10px_40px_rgba(0,0,0,0.6)] space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-[#a0aec0] leading-relaxed font-sans">
                Welcome to the {tierName} UI/UX Prototype Submission panel. Submit your finalized Figma prototype or design file.
              </p>
            </div>
            ...
            <div className="space-y-3 pt-4 border-t border-[#002866]/60">
              ...
              <ul className="space-y-2.5 text-xs text-[#cbd5e0] font-sans">
                <li className="flex items-start gap-2">
                  ...
                  <span>Prototype must be submitted as a valid <strong className="text-white">Figma Link</strong> (<code className="text-[#00d4ff] text-[10px]">figma.com/file/...</code>, <code className="text-[#00d4ff] text-[10px]">figma.com/proto/...</code>, or <code className="text-[#00d4ff] text-[10px]">figma.com/design/...</code>).</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-[#ffb81c] font-bold">•</span>
                  <span>Ensure your Figma file access setting is set to <strong className="text-white">Anyone with the link can view</strong>.</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-[#ffb81c] font-bold">•</span>
